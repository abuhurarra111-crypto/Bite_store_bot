...
def build_button(tpl_id, default_text="🛒 Buy Now", *,
                 callback_data=None, url=None, web_app=None, force_default=False):
    ...
    🐛 v158 FIX: `force_default=True` makes the button use `default_text` even
    if the admin saved a custom text for this template. Broadcast Buy-Now
    buttons pass this so the PRODUCT NAME + premium emoji ALWAYS show on the
    button (owner requirement) instead of a generic saved "Buy Now".
