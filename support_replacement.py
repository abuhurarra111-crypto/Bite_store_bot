...
async def admin_replace_reject_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """🐛 v157 FIX (Bug3): Admin rejected → FIRST ask for a reason (text step),
    then mark rejected + notify the customer WITH the reason. The reason is
    stored on the order so it shows in the replacement history too."""
    q = update.callback_query
    if q.from_user.id != ADMIN_ID:
        await q.answer("❌", show_alert=True); return
    await q.answer()
    ...
    if not o:
        await q.edit_message_text("❌ Order not found."); return
    context.user_data["rej_reason_oid"] = oid
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("⏭ Skip Reason", callback_data=f"adm_reprj_do_{oid}")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"adm_reprj_cancel_{oid}")],
    ])
    try:
        await q.edit_message_text(
...
async def admin_replace_reject_reason_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Receives the rejection reason text → executes the rejection."""
    if update.effective_user.id != ADMIN_ID:
        return False
    oid = context.user_data.pop("rej_reason_oid", None)
    if not oid:
        return False
    reason = (update.message.text or "").strip()[:200] or "Not approved"
    await _do_replacement_reject(update.effective_message, context, int(oid), reason)
    return True


async def admin_replace_reject_do_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Skip-reason button → reject with default reason."""
    q = update.callback_query
    if q.from_user.id != ADMIN_ID:
        await q.answer("❌", show_alert=True); return
    try:
        oid = int(q.data.replace("adm_reprj_do_", ""))
    except Exception:
        await q.answer("Invalid", show_alert=True); return
    context.user_data.pop("rej_reason_oid", None)
    await _do_replacement_reject(q, context, oid, "Not approved")


async def admin_replace_reject_cancel_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    if q.from_user.id != ADMIN_ID:
        await q.answer("❌", show_alert=True); return
    context.user_data.pop("rej_reason_oid", None)
...
async def _do_replacement_reject(target, context, oid, reason):
    """Shared reject executor: mark rejected (with reason) + notify customer."""
    o = get_order(oid)
    if not o:
        try:
            await target.edit_message_text("❌ Order not found.")
        except Exception:
            pass
        return
    try:
        ...
        from database import get_connection
        conn = get_connection(); c = conn.cursor()
        c.execute("UPDATE orders SET replacement_status='rejected', replacement_reason=? WHERE id=?",
                  (str(reason)[:200], int(oid)))
        conn.commit(); conn.close()
    except Exception:
    ...
    try:
        await target.edit_message_text(
            ...
            f"Order #{oid} — customer notified with reason.\n"
            f"📝 Reason: *{escape_md(reason)}*",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([
...
# ════════════════════════════════════════════════════════════
# 🆕 v158 — TICKET CLOSE REMINDER (3-hour) + USER-INITIATED CLOSE
# The 30-minute auto-close is GONE. Tickets stay open until the USER closes
# them. Every 3 hours the bot reminds the user with a "🔒 Close Ticket"
# button; when the user taps it, the ticket closes.
# ════════════════════════════════════════════════════════════
def get_due_reminder_tickets(hours=3, limit=25):
    ...
    try:
        ...
        logger.warning(f"[TicketReminder] scan failed: {e}")
        return []
    from datetime import datetime, timedelta
    now = datetime.now()
    due = []
    for t in rows:
        try:
            last = str(t.get('last_reminder_at') or '').strip()
            created_s = str(t.get('created_at') or '')[:19]
            created_dt = datetime.strptime(created_s, "%Y-%m-%d %H:%M:%S") if created_s else now
            if last:
                last_dt = datetime.strptime(str(last)[:19], "%Y-%m-%d %H:%M:%S")
                if (now - last_dt) > timedelta(hours=hours):
                    due.append(t)
            else:
                if (now - created_dt) > timedelta(hours=hours):
                    due.append(t)
            if len(due) >= limit:
                break
        except Exception:
...
def mark_ticket_reminded(tid):
    """Record that a reminder was sent for this ticket."""
    try:
        from database import get_connection
        from datetime import datetime
        conn = get_connection(); c = conn.cursor()
        c.execute("UPDATE support_tickets SET last_reminder_at=?, reminder_count=COALESCE(reminder_count,0)+1 WHERE id=?",
                  (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), int(tid)))
        conn.commit(); conn.close()
    except Exception:
        pass
...
async def ticket_reminder_job(context):
    ...
    try:
        ...
        logger.warning(f"[TicketReminder] job error: {e}")


async def st_user_close_callback(update, context):
    """🔒 USER taps Close Ticket → ticket closes (only the owner can close
    their own ticket)."""
    q = update.callback_query
    try:
        await q.answer()
        ...
        pass
    try:
        tid = int(q.data.replace("st_uclose_", ""))
    except Exception:
        try:
    ...
    uid = q.from_user.id
    try:
        from database import get_ticket, update_ticket
        t = get_ticket(tid)
        if not t:
            try:
        ...
        try:
            await q.edit_message_text(
                f"🔒 *Ticket #{tid} Closed*\n━━━━━━━━━━━━━━━━━━━━\n\n"
                f"Shukriya! Agar dobara madad chahiye to naya ticket khol dein. 🙏",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
        ...
        try:
            ...
            await context.bot.send_message(ADMIN_ID,
                f"🔒 *Ticket #{tid} closed by user*\nUser: `{uid}`",
                parse_mode="Markdown")
        except Exception:
            pass
    except Exception as e:
        logger.warning(f"[TicketUserClose] #{tid}: {e}")


# 🆕 v158: keep the old helper name as a stub that never returns anything,
# so any legacy import of get_stale_open_tickets won't crash — but the 30-min
# auto-close job is REMOVED from scheduling.
