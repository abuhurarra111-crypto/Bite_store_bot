...
def _format_summary(stats, period_label, rate):
    ...
    profit_pkr = format_pkr(profit, rate)
    net = float(stats.get('net_revenue') or rev_usd)
    lines = [
        "📊 *Analytics Dashboard*",
        "━━━━━━━━━━━━━━━━━━━━",
        ...
        f"📈 *Profit:* ${profit:.2f}  _({profit_pkr})_",
        f"🧾 *Net (after refunds):* ${net:.2f}",
        f"✅ *Delivered Orders:* {stats['delivered']}",
        f"📦 *Total Orders:* {stats['total_orders']}",
        f"⏳ *Pending:* {stats['pending']}",
        f"💸 *Refunds:* {stats.get('refund_count', 0)} (${stats.get('refund_amt', 0):.2f})",
    ...
    if daily:
        lines.append("")
        lines.append("*📅 Daily Revenue (last 7 days):*")
        for d in daily:
            lines.append(f"  `{d['date']}` → ${d['revenue']:.2f} ({d['orders']} orders)")
    return "\n".join(lines)
