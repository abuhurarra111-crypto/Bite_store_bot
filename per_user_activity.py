...
def _attach_buy_emoji(btn, pid, url):
    """🐛 v157 FIX (Bug4): attach the product's premium emoji as the button
    icon (supplier → fixed ext emoji; own → name emoji). Also keeps the color."""
    try:
        from fake_engagement import _product_buy_emoji
        _eid, _ = _product_buy_emoji(int(pid))
        _extras = {}
        if _eid:
            _extras["icon_custom_emoji_id"] = str(_eid)
        if getattr(btn, "api_kwargs", None) and isinstance(btn.api_kwargs, dict):
            _extras.update(btn.api_kwargs)
        if _extras:
            return InlineKeyboardButton(btn.text, url=url, api_kwargs=_extras)
    except Exception:
        pass
    return btn
...
async def build_fake_message(bot, user_id: int) -> tuple[str, any]:
    ...
    if chosen == "stock":
        ...
        if product:
            ...
            try:
                from button_system import build_button as _bb
                _btn = _bb("bc_stock", _lbl, url=_url, force_default=True)
                if _color:
                    try:
                        ...
                        pass
                # 🐛 v157: premium emoji icon (supplier fixed / own name)
                _btn = _attach_buy_emoji(_btn, pid, _url)
            except Exception:
    ...
    if chosen == "newprod":
        ...
        if product:
            ...
            try:
                from button_system import build_button as _bb
                _btn = _bb("bc_newprod", _lbl, url=_url, force_default=True)
                if _color:
                    try:
                        _btn = InlineKeyboardButton(
                            _btn.text, url=_url,
