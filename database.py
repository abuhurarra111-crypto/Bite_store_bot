...
def get_connection():
    ...
    # 🆕 v79 / 🐛 v157: WAL + busy_timeout prevent lock errors. The pragmas
    # are cached — running `journal_mode=WAL` on EVERY new connection was
    # adding real latency to every click (WAL checkpoint lock). Set once per
    # process; WAL is persistent on the DB file anyway.
...
_PRODUCT_COLUMNS = [
    ...
    # 🆕 v157: Bulk Discount feature
    ("discount_pct", "REAL DEFAULT 0"),
    ("discount_until", "TEXT DEFAULT ''"),
]
...
def get_setting(key, default=""):
    """🐛 v157 FIX (Bug5): short-TTL in-memory cache. Handlers call this on
    every update (payment toggles, maint, dest...) — a fresh SQLite query per
    call made the bot feel slow (3-4s on button taps)."""
    now = _stime.time()
...
def invalidate_settings_cache(key=None):
    """Call after set_setting so the next read is fresh."""
    if key is None:
        _SETTINGS_CACHE.clear()
    else:
...
def analytics_summary(days=None):
    """
    Returns analytics dict for a given period — 🐛 v157: REAL calculations.
    - Revenue/profit counted from DELIVERED orders only (price - cost_price)
    - Pending = ALL waiting statuses (binance/usdt/bybit_waiting, screenshot…)
    - Includes refunds (count + amount), net revenue, daily breakdown
    """
    conn = get_connection(); c = conn.cursor()
    where = ""
    if days is not None:
    ...
    # All orders (any status) in period
    c.execute(f"SELECT COUNT(*) FROM orders {where}")
    total_orders = c.fetchone()[0]

    # Pending = every waiting state (REAL pending count)
    pend_states = "('pending','binance_waiting','usdt_waiting','bybit_waiting','screenshot_sent','paid_pending_delivery')"
    c.execute(f"SELECT COUNT(*) FROM orders WHERE status IN {pend_states} {('AND ' + and_where) if and_where else ''}")
    ...
    # Daily breakdown (last 7 days): each day's delivered revenue + order count
    daily = []
    try:
        c.execute("""SELECT substr(created_at,1,10) d, COUNT(*), COALESCE(SUM(price),0)
                     FROM orders WHERE status='delivered'
                     AND created_at >= datetime('now','-6 days')
                     GROUP BY d ORDER BY d DESC""")
        for rr in c.fetchall():
            daily.append({"date": rr[0], "orders": int(rr[1] or 0), "revenue": float(rr[2] or 0)})
    except Exception:
        daily = []
    ...
    return {
        ...
        "net_revenue": revenue - refund_amt,
        "daily": daily,
    }
...
def setup_freebies_tables():
    ...
    """)
    # 🆕 v170.43: max_claims = freebie ka APNA stock (total claims across users;
    # 0 = unlimited). Khatam hote hi auto-disable.
    try:
        ensure_column(c, "freebies", "max_claims", "INTEGER DEFAULT 0")
    except Exception:
        pass
    c.execute("""
...
def get_freebie_config(pid):
    ...
    r = c.fetchone(); conn.close()
    if not r:
        return {"product_id": int(pid), "enabled": 0, "claim_limit": 1,
                "reclaim_refs": 0, "max_claims": 0}
    d = dict(r)
    return {
        ...
        "max_claims": int(d.get("max_claims") or 0),
    }
...
def set_freebie_config(pid, *, enabled=None, claim_limit=None, reclaim_refs=None,
    ...
    try:
        ...
        exists = c.fetchone()
        if not exists:
            c.execute("""INSERT INTO freebies (product_id, enabled, claim_limit, reclaim_refs, max_claims, updated_at)
                         VALUES (?, 0, 1, 0, 0, ?)""",
...
def get_freebie_remaining_claims(pid):
    """max_claims - total_claims (None = unlimited). 0 ya kam = khatam."""
    cfg = get_freebie_config(pid)
    mx = int(cfg.get("max_claims") or 0)
    if mx <= 0:
        return None
    used = get_freebie_total_claims(pid)
...
def record_freebie_claim(user_id, product_id, order_id, refs_used):
    ...
    🆕 v170.43: max_claims set ho to claim ke baad check — limit reach ho jaye
    to freebie AUTO-DISABLE (khatam).
    """
    setup_freebies_tables()
    conn = get_connection(); c = conn.cursor()
    ...
    # auto-disable on max_claims
    try:
        c.execute("SELECT COALESCE(max_claims,0) FROM freebies WHERE product_id=?",
                  (int(product_id),))
        r = c.fetchone()
        mx = int(r[0] or 0) if r else 0
        if mx > 0:
            c.execute("SELECT COUNT(*) FROM freebie_claims WHERE product_id=?",
                      (int(product_id),))
            used = int(c.fetchone()[0] or 0)
            if used >= mx:
                c.execute("UPDATE freebies SET enabled=0, updated_at=CURRENT_TIMESTAMP WHERE product_id=?",
                          (int(product_id),))
    except Exception:
        pass
    conn.commit(); conn.close()
...
# 🆕 v170.43: freebie restock helpers (reuse restock_requests table)
def freebie_restock_request(pid, uid):
    """User ne '🔔 Notify Me' tap kiya (freebie disabled/out-of-stock)."""
    conn = get_connection(); c = conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS restock_requests
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, product_id INTEGER,
                  ...
                  UNIQUE(product_id, user_id))""")
    try:
        c.execute("INSERT INTO restock_requests (product_id, user_id) VALUES (?, ?)",
                  (int(pid), int(uid)))
    except Exception:
        pass
    conn.commit(); conn.close()
...
def freebie_restock_users(pid):
    """Pending 'Notify Me' users for a freebie (then clear)."""
    conn = get_connection(); c = conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS restock_requests
                 (id INTEGER PRIMARY KEY AUTOINCREMENT, product_id INTEGER,
                  user_id INTEGER, created_at TEXT DEFAULT CURRENT_TIMESTAMP,
                  UNIQUE(product_id, user_id))""")
    c.execute("SELECT user_id FROM restock_requests WHERE product_id=?", (int(pid),))
    users = [int(r[0]) for r in c.fetchall()]
    c.execute("DELETE FROM restock_requests WHERE product_id=?", (int(pid),))
    conn.commit(); conn.close()
    return users


# 🆕 v170.43: claim log filters + analytics
def get_all_freebie_claims(limit=50, user_id=None, days=None):
    """Global freebie claim log (filters: user_id, last N days)."""
    setup_freebies_tables()
    conn = get_connection(); c = conn.cursor()
    where = ["1=1"]; params = []
    if user_id:
        where.append("fc.user_id=?"); params.append(int(user_id))
    if days:
        from datetime import datetime, timedelta
        cutoff = (datetime.now() - timedelta(days=int(days))).strftime("%Y-%m-%d %H:%M:%S")
        where.append("fc.claimed_at>=?"); params.append(cutoff)
    c.execute(f"""SELECT fc.*, u.username, u.first_name, p.name AS product_name
                 FROM freebie_claims fc
                 LEFT JOIN users u ON u.user_id = fc.user_id
                 LEFT JOIN products p ON p.id = fc.product_id
                 WHERE {' AND '.join(where)}
                 ORDER BY fc.id DESC LIMIT ?""", params + [int(limit)])
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows


def get_freebies_analytics(days=7):
    """🆕 v170.43: last N days ke freebie claims (per-day) + top freebies."""
    setup_freebies_tables()
    from datetime import datetime, timedelta
    conn = get_connection(); c = conn.cursor()
    # per-day
    ...
    for i in range(int(days) - 1, -1, -1):
        ...
        c.execute("""SELECT COUNT(*) FROM freebie_claims
                     WHERE date(claimed_at)=?""", (d,))
        daily.append({"date": d[5:], "count": int(c.fetchone()[0] or 0)})
    # top freebies (total)
    c.execute("""SELECT p.name, p.id, COUNT(fc.id) n
                 FROM freebie_claims fc
                 LEFT JOIN products p ON p.id = fc.product_id
                 GROUP BY fc.product_id ORDER BY n DESC LIMIT 5""")
    top = [dict(r) for r in c.fetchall()]; conn.close()
...
def set_bulk_discount(product_ids, pct, hours=0):
    """Apply discount_pct to many products at once. hours=0 → no expiry."""
    try:
        from datetime import datetime, timedelta
        conn = get_connection(); c = conn.cursor()
        until = ""
        if hours and float(hours) > 0:
            until = (datetime.utcnow() + timedelta(hours=float(hours))).strftime("%Y-%m-%d %H:%M:%S")
        for pid in (product_ids or []):
            try:
                c.execute("UPDATE products SET discount_pct=?, discount_until=? WHERE id=?",
                          (float(pct or 0), until, int(pid)))
            except Exception:
                pass
        conn.commit(); conn.close()
        return True
...
def clear_bulk_discount(product_ids=None):
    """Remove discounts (optionally only for the given ids)."""
    try:
        conn = get_connection(); c = conn.cursor()
        if product_ids:
            for pid in product_ids:
                try:
                    c.execute("UPDATE products SET discount_pct=0, discount_until='' WHERE id=?", (int(pid),))
                except Exception:
                    pass
        else:
            c.execute("UPDATE products SET discount_pct=0, discount_until=''")
        conn.commit(); conn.close()
        return True
    except Exception:
        return False
...
def get_discounted_products():
    """All products currently on discount (active)."""
    try:
        from datetime import datetime
        conn = get_connection(); conn.row_factory = DictRow
        c = conn.cursor()
        c.execute("SELECT * FROM products WHERE COALESCE(discount_pct,0) > 0")
        rows = [dict(r) for r in c.fetchall()]
        conn.close()
        out = []
        for r in rows:
            until = str(r.get("discount_until") or "").strip()
            if until:
                try:
                    if datetime.utcnow() > datetime.strptime(until, "%Y-%m-%d %H:%M:%S"):
                        continue
...
# ════════════════════════════════════════════════════════════════
# 📊 v158: TIERED QUANTITY DISCOUNTS (per-product bulk pricing)
# Owner wants: "Gemini — 1 qty = $1, 10 qty = $0.89, 30 = $0.52, 50 = $0.45"
# Each product can have MULTIPLE quantity tiers. The shop shows them in the
# product description, and checkout auto-applies the price for the qty chosen.
# ════════════════════════════════════════════════════════════════

def ensure_tier_discount_table(c=None):
    try:
        if c is None:
            conn = get_connection(); c = conn.cursor()
            own = True
            ...
            own = False
        c.execute("""
            CREATE TABLE IF NOT EXISTS product_tier_discounts (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
                product_id  INTEGER NOT NULL,
...
def remove_product_tier(product_id, min_qty):
    try:
        ensure_tier_discount_table()
        conn = get_connection(); c = conn.cursor()
        c.execute("DELETE FROM product_tier_discounts WHERE product_id=? AND min_qty=?",
                  (int(product_id), int(min_qty)))
        conn.commit(); conn.close()
        return True
    except Exception:
        return False
...
def clear_product_tiers(product_id):
    try:
        ensure_tier_discount_table()
        conn = get_connection(); c = conn.cursor()
        c.execute("DELETE FROM product_tier_discounts WHERE product_id=?", (int(product_id),))
        conn.commit(); conn.close()
        return True
    except Exception:
        return False
        ...
        for t in tiers:
            if int(t["min_qty"]) <= qty:
                if best is None or int(t["min_qty"]) > int(best["min_qty"]):
                    best = t
        if best:
            return float(best["unit_price"]), int(best["min_qty"])
        return float(base_price), 1
    except Exception:
        return float(base_price), 1
...
def product_tiers_text(product, base_price=None, rate=0, mode="md"):
    """v160: editable tiered-discount block shown on the product page.

    Uses the admin-editable templates `tier_display` (block) and `tier_line`
    (per-tier line) from Templates editor — placeholders:
      tier_display: {product}, {base_price}, {tiers}, {currency}
      tier_line:    {qty}, {price}, {product}, {base_price}
    Premium emojis are supported ([[HTML]]<tg-emoji> markup).

    `mode`: "md" returns Markdown (for the Markdown page branch);
            "html" converts to HTML (for the HTML page branch) — this fixes
            the ugly literal `**` asterisks that appeared before.
    """
    try:
        d = dict(product)
        pid = int(d.get("id") or 0)
        ...
        try:
            from customization import get_template as _gt, render_template as _rt
        except Exception:
            _gt = _rt = None

        def _fill(tpl_id, data):
            """Render by TEMPLATE ID (render_template expects the id, not text)."""
            try:
                if _rt:
                    return _rt(tpl_id, data)
                return _gt(tpl_id)
            except Exception:
                return _gt(tpl_id)

        lines = []
        # always build the per-tier lines (custom line template or default)
        for t in tiers:
            l = _fill("tier_line", {
                "qty": int(t["min_qty"]),
                "price": f"{float(t['unit_price']):.2f}",
        ...
        if not lines:
            for t in tiers:
                lines.append(f"🎉 {int(t['min_qty'])}+ qty → 💵 **${float(t['unit_price']):.2f}** each")
        tiers_txt = "\n".join(lines)

        block = _fill("tier_display", {
            "product": pname,
            "base_price": f"{float(base_price):.2f}",
            "tiers": tiers_txt,
            "currency": "$",
        ...
        if not block or not str(block).strip():
            block = f"📊 *Bulk Discounts — Quantity Pricing*\n{tiers_txt}"

        if str(mode).lower() == "html":
            try:
                from utils import markdownish_to_html, sanitize_html_tags
                out = sanitize_html_tags(markdownish_to_html(str(block)))
                # polish: remove any stray single * / _ left after conversion
                import re as _re
                out = _re.sub(r"\*+", "", out)
                out = _re.sub(r"(?<![A-Za-z0-9])_(?![A-Za-z0-9])", "", out)
                return out
            except Exception:
                return str(block)
        return str(block)
...
# ════════════════════════════════════════════════════════════════
# 🆕 v161: RESELLER API (ProdSeller-compatible) — tables + helpers
# ════════════════════════════════════════════════════════════════

def migrate_reseller_tables():
    """v161: create reseller_orders + products.reseller_* columns.
    v161.2: per-key pricing/security/webhook columns + file delivery refs.
    Idempotent — safe on every boot and after DB restore."""
    try:
        setup_api_tables()
    except Exception:
        pass
    conn = get_connection(); c = conn.cursor()
    try:
        c.execute("""CREATE TABLE IF NOT EXISTS reseller_orders (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        c.execute("CREATE INDEX IF NOT EXISTS idx_reseller_orders_user ON reseller_orders(user_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_reseller_orders_key ON reseller_orders(key_id)")
        ensure_column(c, "reseller_orders", "delivery_file_id",   "TEXT DEFAULT ''")
        ensure_column(c, "reseller_orders", "delivery_file_name", "TEXT DEFAULT ''")
        ensure_column(c, "reseller_orders", "delivery_file_type", "TEXT DEFAULT ''")
        ensure_column(c, "reseller_orders", "webhook_sent",       "INTEGER DEFAULT 0")
        ensure_column(c, "products", "reseller_enabled", "INTEGER DEFAULT 1")
        ensure_column(c, "products", "reseller_price",   "REAL DEFAULT 0")
        # v161.2 — per-key reseller config (pricing, security, webhook)
        ensure_column(c, "api_keys", "reseller_markup",    "REAL")           # NULL = global
        ensure_column(c, "api_keys", "reseller_base_mode", "TEXT DEFAULT ''")  # ''|cost|price
        ensure_column(c, "api_keys", "webhook_url",        "TEXT DEFAULT ''")
        ...
        c.execute("INSERT OR IGNORE INTO bot_settings (key, value) VALUES ('reseller_base_mode', 'price')")
        conn.commit()
    except Exception:
        try: conn.rollback()
        except Exception: pass
...
def create_reseller_order(key_id, user_id, product_id, product_name, qty,
                          usd_amount, points_amount, status="pending", idem_key=""):
    from datetime import datetime
    conn = get_connection(); c = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute("""INSERT INTO reseller_orders
        (key_id, user_id, product_id, product_name, qty, usd_amount,
         points_amount, status, idem_key, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (int(key_id or 0), int(user_id or 0), int(product_id or 0),
         str(product_name or "")[:200], int(qty or 1), float(usd_amount or 0),
         float(points_amount or 0), str(status or "pending"), str(idem_key or "")[:64], now))
    oid = c.lastrowid
    conn.commit(); conn.close()
    return oid


def update_reseller_order(order_id, **fields):
    if not fields:
        return
    from datetime import datetime
    allowed = {"status", "delivery_text", "delivered_keys", "remote_order_id",
               "error", "delivered_at", "points_amount", "usd_amount",
               "delivery_file_id", "delivery_file_name", "delivery_file_type",
               "webhook_sent"}
    sets, vals = [], []
    ...
    conn = get_connection(); c = conn.cursor()
    try:
        c.execute(f"UPDATE reseller_orders SET {', '.join(sets)} WHERE id=?", vals)
        conn.commit()
    except Exception:
        try: conn.rollback()
        except Exception: pass
...
def get_reseller_order(order_id):
    conn = get_connection(); c = conn.cursor()
    c.execute("SELECT * FROM reseller_orders WHERE id=?", (int(order_id),))
    r = c.fetchone(); conn.close()
    return dict(r) if r else None


def list_reseller_orders(user_id=None, limit=50):
    conn = get_connection(); c = conn.cursor()
    if user_id:
        c.execute("SELECT * FROM reseller_orders WHERE user_id=? ORDER BY id DESC LIMIT ?",
                  (int(user_id), int(limit)))
    else:
        c.execute("SELECT * FROM reseller_orders ORDER BY id DESC LIMIT ?", (int(limit),))
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows


# ────────────────────────────────────────────────────────────
# 🆕 v161.2 — RESELLER KEY CONFIG + MANUAL DELIVERY HELPERS
# ────────────────────────────────────────────────────────────

def get_api_key_row(key_id: int):
    """Full api_keys row (incl. v161.2 reseller config columns)."""
    conn = get_connection(); c = conn.cursor()
    c.execute("SELECT * FROM api_keys WHERE id=?", (int(key_id),))
    r = c.fetchone(); conn.close()
    return dict(r) if r else None


def update_api_key_fields(key_id: int, **fields):
    """Update allowed api_keys columns. Returns True if a row changed."""
    allowed = {"reseller_markup", "reseller_base_mode", "webhook_url",
               "spend_limit_usd", "allowed_products", "ip_whitelist", "label",
               "rate_limit", "webhook_secret"}
    sets, vals = [], []
    for k, v in fields.items():
        if k in allowed:
            sets.append(f"{k}=?")
...
def complete_reseller_order(order_id: int, delivery_text: str, delivered_keys=None):
    """Admin manual delivery for a pending reseller order.
    Sets status=delivered + delivery content. Returns True on success."""
    from datetime import datetime
    oid = int(order_id)
    conn = get_connection(); c = conn.cursor()
    try:
        c.execute("SELECT * FROM reseller_orders WHERE id=?", (oid,))
        r = c.fetchone()
        if not r:
            return False
        keys = delivered_keys
        if keys is None:
            keys = [str(delivery_text)] if str(delivery_text or "").strip() else []
        import json as _json
        try:
            keys_json = _json.dumps([str(k) for k in keys])
        except Exception:
            keys_json = ""
        c.execute("""UPDATE reseller_orders
                     SET status='delivered', delivery_text=?, delivered_keys=?,
                         delivered_at=?, error=''
                     WHERE id=? AND status IN ('pending','paid_pending_delivery')""",
                  (str(delivery_text or ""), keys_json,
...
def reseller_key_total_spent(key_id: int) -> float:
    """Total USD charged on a key (delivered + pending) — for spend limits."""
    conn = get_connection(); c = conn.cursor()
    try:
        c.execute("""SELECT COALESCE(SUM(usd_amount),0) FROM reseller_orders
                     WHERE key_id=? AND status IN ('delivered','pending')""", (int(key_id),))
        v = c.fetchone()[0]
        return float(v or 0)
    except Exception:
        return 0.0
...
def reseller_key_orders(key_id: int, limit=20) -> list:
    """Recent orders for one key (admin view + webhook context)."""
    conn = get_connection(); c = conn.cursor()
    c.execute("SELECT * FROM reseller_orders WHERE key_id=? ORDER BY id DESC LIMIT ?",
              (int(key_id), int(limit)))
    rows = [dict(r) for r in c.fetchall()]; conn.close()
    return rows


def mark_reseller_webhook_sent(order_id: int) -> None:
    conn = get_connection(); c = conn.cursor()
    try:
        c.execute("UPDATE reseller_orders SET webhook_sent=1 WHERE id=?", (int(order_id),))
        conn.commit()
    except Exception:
        try: conn.rollback()
        except Exception: pass
