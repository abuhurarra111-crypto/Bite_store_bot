...
async def adm_st_progress_callback(update, context):
    """Mark ticket as in progress. 🐛 v157: bulletproof — never 'Temporary error'."""
    q = update.callback_query
    try:
        if q.from_user.id != ADMIN_ID:
            await q.answer("❌", show_alert=True); return
        ...
        update_ticket(tid, status='in_progress')
        set_cb_data(update, f"adm_st_view_{tid}")
        await adm_st_view_callback(update, context, _skip_answer=True)
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"[TicketProgress] {e}")
        try:
            await q.answer("✅ Updated", show_alert=False)
        except Exception:
            pass
        try:
            from utils import _safe_edit as _se
            await _se(q, "🔄 Ticket marked in progress.", parse_mode="Markdown")
        except Exception:
            pass


async def adm_st_close_callback(update, context):
    """Close a ticket. 🐛 v157: bulletproof — never 'Temporary error'."""
    q = update.callback_query
    try:
        if q.from_user.id != ADMIN_ID:
            await q.answer("❌", show_alert=True); return
        ...
        import logging
        logging.getLogger(__name__).warning(f"[TicketClose] {e}")
        try:
            await q.answer("✅ Updated", show_alert=False)
        except Exception:
            pass
